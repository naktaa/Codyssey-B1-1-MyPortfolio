...
체크 표시는 코드 구현 기준입니다. 필수 웹 기능과 선택 기능인 프로젝트 언어 필터를 구현했습니다. 배포 환경의 최종 검증과 제출 자료 준비는 별도로 진행합니다.
...
- [x] 빈 값 검증
- [x] 이메일 형식 검증
- [x] 오류 메시지를 각 입력 필드 근처에 표시
- [x] `input` 이벤트로 오류 상태 갱신
- [x] 제출 시 `preventDefault()`
- [x] 유효한 제출 시 성공 메시지 표시
...
- [x] `forEach()` 활용
- [x] `filter()`는 필수 아님. 프로젝트 필터링 구현 시 활용
...
프로젝트 언어별 필터까지만 구현하고 사용자 확인을 마쳤다. 나머지 보너스는 현재 범위에서 진행하지 않는다.

- [x] GitHub 저장소 언어별 필터링
  - [x] 필터 버튼
  - [x] `Array.filter()` 사용
- [ ] Hero 타이핑 효과
...
- GitHub 사용자 아이디: `naktaa`
- 포트폴리오 표시 이름: 손현석
- 소개 문구: 웹 기본 원리, 직접 구현, 학습 기록 중심
...
- 보너스 구현 범위: GitHub 프로젝트 언어별 필터만 완료
