...
> 현재 상태: **필수 웹 기능 구현 완료, Stage 9~10 제출·배포 준비 중**
>
> 이름·소개·GitHub 링크와 프로필 이미지를 반영했습니다. 선택 기능은 프로젝트 언어 필터까지만 구현하고 사용자 확인을 마쳤습니다. 다크 모드 색상 최종 조정, 스크린샷, GitHub Pages 배포와 배포 환경 전체 검증이 남아 있습니다.
...
- GitHub 저장소 언어 목록으로 생성하는 프로젝트 필터
...
포트폴리오 이름은 손현석이며, 학습 과정과 구현 원칙을 소개 문구에 반영했습니다. About에는 엄지를 든 고양이 프로필 이미지를, Footer에는 GitHub 프로필 링크를 제공합니다.
...
Projects는 naktaa의 공개 저장소를 최근 업데이트 순으로 최대 100개 표시합니다. 카드에는 이름·설명·언어·별 수·GitHub 링크가 있으며 Grid가 화면 폭에 맞춰 열을 나눕니다. 응답에 포함된 언어로 필터 버튼을 만들며, 선택한 언어의 저장소만 표시할 수 있습니다. 로딩/빈 목록/오류 안내와 재시도 버튼을 제공합니다.
...
필터 버튼 클릭 → `selectedLanguage` 변경 → `getFilteredProjects()` → `renderProjects()` → 선택 언어 카드와 개수 갱신.

- `projectsState`: 현재 상태, 저장소 배열, 오류 안내, 선택 언어를 보관합니다. 처음의 idle은 요청 전 내부 상태이며 화면은 네 가지 결과 상태로 갱신합니다.
...
1. API 요청이 성공하면 `전체`와 응답에 포함된 언어 버튼이 표시되는지 확인합니다.
2. 언어 버튼을 누르면 해당 언어 카드만 남고 안내의 개수가 일치하는지 확인합니다.
3. `전체`를 누르면 모든 카드가 다시 표시되는지 확인합니다.
4. 선택된 버튼의 색상이 구분되는지, Tab과 Enter/Space로 필터를 사용할 수 있는지 확인합니다.
5. 모바일 390px에서 버튼이 여러 줄로 자연스럽게 배치되고 가로 스크롤이 생기지 않는지 확인합니다.
...
구현 검증에서는 실제 응답을 한 번 조회했고, 이후 성공/빈 목록/403/429/404/500/네트워크/시간 초과/응답 형식 오류와 재시도는 모의 응답으로 검사했습니다. 실제 목록과 상태별 UI, 언어 필터, Description이 없는 카드의 조건부 표시를 사용자가 확인했습니다. GitHub Pages 배포 환경의 최종 통합 검증은 남아 있습니다.
