...
Projects 소개 문구 바로 오른쪽의 'GitHub 저장소 보기' 링크로 전체 저장소에 접근합니다. 좁은 화면에서 공간이 부족하면 링크가 다음 줄 왼쪽에 배치됩니다. 로딩 중에는 안내 옆 스피너가 회전하며 다른 상태가 되면 사라집니다. 동작 줄이기 설정에서는 회전 없이 로딩 문구와 정지된 표시를 제공합니다.
